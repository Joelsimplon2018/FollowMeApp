...
export default class RegistrationFrom {
    ...
    emailAfterDelay(){
        if(!/^\S+@\S+$/.test(this.email.value)){
           this.showValidationError(this.email, "You must provid a valid email adress.")
        }
    ...
    usernameImmediately(){
        ...
        if(this.username.value.length > 30){
            this.showValidationError(this.username, "Username can not be exceed 30 characters")
